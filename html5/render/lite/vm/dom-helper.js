...
export function createBody (vm, type) {
  ...
  return doc.createBody(null, type)
}
...
export function createElement (vm, type) {
  const doc = vm._app.doc
  return doc.createElement(null, type)
}
...
export function attachTarget (vm, target, dest) {
  if (dest.element) {
    // for a fragBlock which has a element.
    ...
    if (target.element) {
      ...
      !target._appended && dest.appendChild(target)
      target.onAppend && target.onAppend()
    }
  }
}
...
function removeElement (target) {
  ...
  if (parent) {
    ...
    target.onRemove && target.onRemove()
  }
}
